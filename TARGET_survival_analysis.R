...
# 1) File di mapping
mapping_file <- "/home/sergio/Scrivania/gmNK/TARGET_analysis/fileid_barcode_association.csv"
...
# Funzione per estrarre TPM CD274
...
base_path <- "/home/sergio/Scrivania/gmNK/TARGET_analysis/GDCdata"
tumor_dirs <- list.dirs(base_path, recursive = FALSE, full.names = TRUE)

# Directory per salvare i plot
plot_dir <- "/home/sergio/Scrivania/gmNK/TARGET_analysis/Survivalplots"
...
for (tumor_path in tumor_dirs) {
  ...
  if(nrow(merged_data) > 0){
    merged_data <- merged_data %>%
      mutate(CD274_group = ifelse(CD274_tpm >= median(CD274_tpm), "High", "Low"))
    ...
    ggsave(filename = file.path(plot_dir, paste0("CD274_OS_", tumor_name, ".png")),
    ...
    if (nrow(efs_data) > 0) {
      ...
      efs_plot <- ggsurvplot(efs_fit, data = efs_data, pval = TRUE, risk.table = TRUE,
                             ...
                             legend.title = "CD274 group")
      ggsave(filename = file.path(plot_dir, paste0("CD274_EFS_", tumor_name, ".png")),
             plot = efs_plot$plot, width = 5, height = 5, dpi = 300)
    } else {
    ...
    if (nrow(dfs_data) > 0) {
      ...
      dfs_plot <- ggsurvplot(dfs_fit, data = dfs_data, pval = TRUE, risk.table = TRUE,
                             ...
                             legend.title = "CD274 group")
      ggsave(filename = file.path(plot_dir, paste0("CD274_DFS_", tumor_name, ".png")),
             plot = dfs_plot$plot, width = 5, height = 5, dpi = 300)
    } else {
