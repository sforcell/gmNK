...
# Definizione:
# La median OS (sopravvivenza mediana globale) è il tempo a cui il 50% dei pazienti è deceduto.
# Se meno del 50% è deceduto, la mediana non è raggiunta.

library(tidyverse)
library(xml2)
library(survival)

# === 1. Carica mapping file ===
...
extract_expression <- function(tsv_file, mapping_df, gene_symbol = "HLA-E") {
...
for (tumor_path in tumor_dirs) {
  ...
  expression_data <- bind_rows(
    lapply(expression_files, extract_expression, mapping_df = mapping_df, gene_symbol = "HLA-E")
  ) %>% filter(!is.na(TPM))

  merged_data <- inner_join(clinical_data, expression_data, by = "patient_id")

  if (nrow(merged_data) < 10) {
    cat("Dati insufficienti (n <", nrow(merged_data), "), salto.\n")
    next
  }
...
if (nrow(summary_df) > 0) {
  p <- ggplot(summary_df, aes(x = tumor, y = median_OS, fill = group)) +
    ...
    labs(title = "Kaplan-Meier Estimated Median OS by HLA-E Expression",
         x = "Tumor Type",
         y = "Median OS (days)",
  ...
  ggsave(file.path(plot_dir, "Kaplan_Meier_Median_OS_HLAE_barplot.pdf"), p, width = 10, height = 5)
} else {
  cat("Nessun dato disponibile per il plot.\n")
}
