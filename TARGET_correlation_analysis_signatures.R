...
output_dir <- "/home/sergio/Scrivania/gmNK/TARGET_analysis/correlation_scatterplots_signatures_immunosuppression_infiltration"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Geni target e signature
gene1 <- "CD274"
gene2 <- "HLA-E"
...
# Funzione per leggere il TPM da singolo file
...
# Funzione per calcolare lo score di una signature
...
# Lista per accumulare correlazioni
correlation_list <- list()

# Loop per ciascun tumore
...
for (tumor in tumors) {
  ...
  # Classificazione hot/intermediate/cold
  ...
  for (gene in c(gene1, gene2)) {
    ...
    for (type in c("Infiltration", "Immunosuppression")) {
      ...
      # Salva correlazione per heatmap
...
if (length(correlation_list) > 0) {
  ...
  # Maschera valori non significativi
  cor_matrix_masked <- cor_matrix
  cor_matrix_masked[pval_matrix > 0.05] <- NA

  # Rimuove righe e colonne completamente NA
  ...
  # Plot heatmap senza scritte
  pdf(file.path(output_dir, "TARGET_heatmap_correlations.pdf"), width = 10, height = 2.5)
  pheatmap(cor_matrix_masked,
           cluster_rows = FALSE,
           cluster_cols = FALSE,
           fontsize = 12,
           main = "TARGET database",
           na_col = "white")  # Ruota le etichette delle colonne di 45 gradi
  dev.off()

  cat("Heatmap salvata in:", file.path(output_dir, "TARGET_heatmap_correlations.pdf"), "\n")
}
