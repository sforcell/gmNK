...
plot_dir <- "/home/sergio/Scrivania/gmNK/TCGA_analysis/correlation_plots_CD274_HLAE"
...
# Function: Read TPM from a sample file
...
# Function: Scatter plot for gene correlation
plot_gene_correlation <- function(expr_mat, gene1, gene2, tumor_name, plot_dir) {
  if (!(gene1 %in% rownames(expr_mat)) || !(gene2 %in% rownames(expr_mat))) {
    cat("Genes not found in", tumor_name, "\n")
    return(NULL)
  }
  ...
  # Order samples by CD274 expression
  ...
  # Plot (CD274 circles in background, HLA-E triangles in foreground)
  p <- ggplot(df, aes(x = Sample)) +
    # CD274 in the background
    geom_point(aes(y = CD274, color = "CD274"), size = 2.5) +
    # HLA-E in the foreground
  ...
  cat("Final plot saved for", tumor_name, "\n")
}
...
for (tumor in tumors) {
  ...
  # Plot only if both genes are present
