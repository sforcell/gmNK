...
extract_clinical <- function(xml_file) {
  ...
  # EFS: first event between recurrence or death
  ...
  # DFS: only recurrence is an event, death is censored
...
extract_expression <- function(tsv_file, mapping_df) {
  ...
  cd274_row <- data %>% filter(gene_name == "CD274")
  if(nrow(cd274_row) == 0) return(NULL)
  tibble(
    patient_id = barcode_row$barcode[1],
    CD274_tpm = cd274_row$tpm_unstranded[1]
  )
}

# 2) Folder containing all tumors
...
for (tumor_path in tumor_dirs) {
  ...
  if(nrow(merged_data) > 0){
    # Compute 20% and 80% quantiles and define groups
    merged_data <- merged_data %>%
      mutate(
        quantile_20 = quantile(CD274_tpm, 0.20),
        quantile_80 = quantile(CD274_tpm, 0.80),
        CD274_group = case_when(
          CD274_tpm < quantile_20 ~ "Low",
          CD274_tpm > quantile_80 ~ "High",
          TRUE ~ "Intermediate"
        )
      ) %>%
    ...
    os_fit <- survfit(os_surv ~ CD274_group, data = merged_data)
    ...
    ggsave(filename = file.path(plot_dir, paste0("CD274_OS_", tumor_name, ".pdf")),
    ...
    if (nrow(efs_data) > 0) {
      ...
      efs_fit <- survfit(efs_surv ~ CD274_group, data = efs_data)
      ...
      ggsave(filename = file.path(plot_dir, paste0("CD274_EFS_", tumor_name, ".pdf")),
             plot = efs_plot$plot, width = 4, height = 4, dpi = 300)
    } else {
      cat("  → Not enough data for EFS.\n")
    }
    ...
    if (nrow(dfs_data) > 0) {
      ...
      dfs_fit <- survfit(dfs_surv ~ CD274_group, data = dfs_data)
      ...
      ggsave(filename = file.path(plot_dir, paste0("CD274_DFS_", tumor_name, ".pdf")),
             plot = dfs_plot$plot, width = 4, height = 4, dpi = 300)
    } else {
      cat("  → Not enough data for DFS.\n")
    }
