...
output_dir <- "/home/sergio/Scrivania/gmNK/TCGA_analysis/correlation_results"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

target_genes <- c("CD274", "HLA-E")
tumors <- list.dirs(base_dir, recursive = FALSE, full.names = FALSE)

# Funzione: Lettura e filtro per protein_coding
...
run_go_analysis <- function(gene_symbols, out_file_prefix) {
  ...
  entrez_ids <- tryCatch({
    ...
    cat("Errore nella conversione SYMBOL -> ENTREZID:", conditionMessage(e), "\n")
    return(NULL)
  })
  
  if (is.null(entrez_ids) || nrow(entrez_ids) == 0) {
    cat("Nessun ID ENTREZ valido trovato per:", out_file_prefix, "\n")
    return(NULL)
  }
  ...
  if (nrow(go_df) == 0) {
    cat("Nessun termine GO significativo per:", out_file_prefix, "\n")
    return(NULL)
  }
...
for (tumor in tumors) {
  ...
  if (length(genes_found) == 0) {
    cat("Geni target non trovati in:", tumor, "\n")
    next
  }
  
  for (target in genes_found) {
    cat("Correlazione con gene:", target, "\n")
    ...
    if (nrow(pos_cor) > 0 && any(!is.na(pos_cor$gene))) {
      cat("Analisi GO per positivi (n =", nrow(pos_cor), ")\n")
      run_go_analysis(pos_cor$gene, paste0(prefix, "_positive"))
    } else {
      cat("Nessun gene positivo significativo per GO\n")
    }
    
    if (nrow(neg_cor) > 0 && any(!is.na(neg_cor$gene))) {
      cat("Analisi GO per negativi (n =", nrow(neg_cor), ")\n")
      run_go_analysis(neg_cor$gene, paste0(prefix, "_negative"))
    } else {
      cat("Nessun gene negativo significativo per GO\n")
    }
    
    cat("Salvati:", nrow(pos_cor), "positivi e", nrow(neg_cor), "negativi + GO + grafici (se presenti)\n")
  }
}
