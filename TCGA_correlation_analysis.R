...
# Function: Read and filter for protein_coding genes
...
# Function: GO analysis + plots (robust version)
run_go_analysis <- function(gene_symbols, out_file_prefix) {
  ...
  entrez_ids <- tryCatch({
    ...
    cat("Error converting SYMBOL -> ENTREZID:", conditionMessage(e), "\n")
    return(NULL)
  })
  ...
  if (nrow(go_df) == 0) {
    cat("No significant GO terms for:", out_file_prefix, "\n")
    return(NULL)
  }
...
for (tumor in tumors) {
  ...
  if (length(expr_list) < 3) {
    cat("Insufficient data for:", tumor, "\n")
    next
  }
  ...
  for (target in genes_found) {
    cat("Correlation with gene:", target, "\n")
    ...
    if (nrow(pos_cor) > 0 && any(!is.na(pos_cor$gene))) {
      cat("GO analysis for positive genes (n =", nrow(pos_cor), ")\n")
      run_go_analysis(pos_cor$gene, paste0(prefix, "_positive"))
    } else {
      cat("No significant positive genes for GO\n")
    }
    
    if (nrow(neg_cor) > 0 && any(!is.na(neg_cor$gene))) {
      cat("GO analysis for negative genes (n =", nrow(neg_cor), ")\n")
      run_go_analysis(neg_cor$gene, paste0(prefix, "_negative"))
    } else {
      cat("No significant negative genes for GO\n")
    }
